...
class IocStateManager:
    def __init__(self, dir_path, verbose, **kwargs):
        ...
        if not dir_path or not os.path.isdir(dir_path):
            print(
                f'IocStateManager.__init__: Incorrect initialization parameters: dir_path="{dir_path}".'
            )
    ...
    def read_config(self, create):
        if os.path.exists(self.info_file_path):
            try:
                ...
                if self.verbose:
                    print(
                        f'IocStateManager.read_config: Failed to read state info file "{self.info_file_path}", {e}.'
                    )
                self.create_error()
                state_info = f"state info file lost."
                prompt = f'state info file "{self.info_file_path}" lost.'
                self.set_state_info(
                    state=STATE_ERROR, state_info=state_info, prompt=prompt
                )
            else:
                if self.verbose:
                    print(
                        f'IocStateManager.read_config: Read state info file "{self.info_file_path}".'
                    )
        else:
            if create:
                self.create_new()
                if self.verbose:
                    print(
                        f'IocStateManager.read_config: Create state info file "{self.info_file_path}".'
                    )
            else:
                self.create_error()
                state_info = f"state info file lost."
...
class IOC:
    def __init__(self, dir_path=None, read_mode=False, verbose=False, **kwargs):
        ...
        if verbose:
            if read_mode:
                print(
                    f'IOC.__init__: Start initializing at "{dir_path}" in read-only mode.'
                )
            else:
                print(f'IOC.__init__: Start initializing at "{dir_path}".')
        if not dir_path or not os.path.isdir(dir_path):
            print(
                f'IOC.__init__: Invalid initialization parameters: dir_path="{dir_path}".'
            )
            return
        ...
        self.state_manager = IocStateManager(
            dir_path=kwargs.get("state_info_ini_dir", self.dir_path),
        ...
        if self.name != os.path.basename(self.dir_path):
            ...
            print(
                f'IOC.__init__: Set attribute "self.name" from "{old_name}" to "{self.name}" '
        ...
        self.dir_path_for_mount = os.path.join(
            MOUNT_PATH,
            self.get_config("host") if self.get_config("host") else "swarm",
            self.name,
        )
        self.config_file_path_for_mount = os.path.join(
            self.dir_path_for_mount, IOC_CONFIG_FILE
        )
        self.startup_path_for_mount = os.path.join(self.dir_path_for_mount, "startup")

        self.settings_path_in_docker = os.path.join(
            CONTAINER_IOC_RUN_PATH, self.name, "settings"
        )
        self.logs_path_in_docker = os.path.join(
            CONTAINER_IOC_RUN_PATH, self.name, "logs"
        )
        self.startup_path_in_docker = os.path.join(
            CONTAINER_IOC_RUN_PATH, self.name, "startup"
        )
    ...
    def read_config(self, create):
        if os.path.exists(self.config_file_path):
            ...
            if create:
                ...
                if self.verbose:
                    print(
                        f'IOC.read_config: Create config file "{self.config_file_path}" with default settings.'
                    )
            else:
    ...
    def add_module_template(self, template_type):
        if template_type.lower() == "asyn":
            ...
            if not self.conf.has_section("RAW"):
                self.add_raw_cmd_template()
            cmd_before_dbload = (
                f'epicsEnvSet("STREAM_PROTOCOL_PATH", {self.startup_path_in_docker})\n'
                f'drvAsynIPPortConfigure("L0","192.168.0.23:4001",0,0,0)\n'
                f'drvAsynSerialPortConfigure("L0","/dev/tty.PL2303-000013FA",0,0,0)\n'
                f'asynSetOption("L0", -1, "baud", "9600")\n'
                f'asynSetOption("L0", -1, "bits", "8")\n'
    ...
    def add_raw_cmd_template(self):
        ...
        if self.conf.has_section(sc):
            print(
                f'IOC("{self.name}").add_raw_cmd_template: Failed. Section "{sc}" already exists.'
            )
            return False
    ...
    def get_src_file(
        self, src_dir=None, init_mode=False, print_info=False, no_exec=False
    ):
        if no_exec:
            return
        ...
        if not os.path.isdir(self.src_path):
            print(
                f'IOC("{self.name}").get_src_file: Failed. Source path of project "{self.src_path}" not exist.'
            )
            state_info = "source directory lost."
            prompt = "source directory was lost and an empty dir was created."
            self.state_manager.set_state_info(
                STATE_ERROR, state_info=state_info, prompt=prompt
            )
        ...
        if init_mode:
            ...
            file_list = (
                [item.strip() for item in filter(None, db_list.split(","))]
        ...
        for item in os.listdir(src_p):
            if item.endswith(DB_SUFFIX):
                if item not in file_list:
                    ...
                    if src_p != self.src_path:
                        file_copy(
                            os.path.join(src_p, item),
                            os.path.join(self.src_path, item),
                            "rw",
                            self.verbose,
                        )
                        file_flag = True
        ...
        if proto_list:
            ...
            if self.verbose or print_info:
                print(
                    f'IOC("{self.name}").get_src_file: Managed protocol files "{proto_list}".'
                )
        if others_list:
            self.set_config("others_file", others_list, "SRC")
            if self.verbose or print_info:
                print(
                    f'IOC("{self.name}").get_src_file: Managed other files "{others_list}".'
                )
    ...
    def generate_substitutions_file(self):
        ...
        for load_line in multi_line_parse(self.get_config("load", "DB")):
            ...
            if db_file not in os.listdir(self.src_path):
                ...
                self.state_manager.set_state_info(
                    state=STATE_WARNING, state_info=state_info, prompt=prompt
                )
                print(
                    f'IOC("{self.name}").generate_substitutions_file: Failed. DB file "{db_file}" not found in '
                    f'path "{self.src_path}" while parsing string "{load_line}" in "load" option.'
                )
                return False
            else:
                file_copy(
                    os.path.join(self.src_path, db_file),
                    os.path.join(self.db_path, db_file),
                    "r",
                    self.verbose,
                )
            ks = ""
            ...
            for c in conditions:
                ...
                if k:
                    ...
                    print(
                        f'IOC("{self.name}").generate_substitutions_file: Failed. Bad load string '
        ...
        if lines_to_add:
            ...
            try:
                ...
                self.state_manager.set_state_info(
                    state=STATE_WARNING, state_info=state_info, prompt=f"{e}"
                )
                print(
                    f'IOC("{self.name}").generate_substitutions_file: Failed. '
                ...
                if self.verbose:
                    print(
                        f'IOC("{self.name}").generate_substitutions_file: Create "{self.name}.substitutions".'
                    )
    ...
    def generate_startup_files(self):
        ...
        if not self.generate_check():
            print(
                f'IOC("{self.name}").generate_startup_files": Failed. Checks failed before generating startup files.'
            )
            return

        lines_before_dbload = []
        lines_at_dbload = [
            f"cd {self.startup_path_in_docker}\n",
        ...
        lines_before_dbload.append(
            f"#!{CONTAINER_IOC_PATH}/{bin_IOC}/bin/linux-x86_64/{bin_IOC}\n\n"
        )

        # source envPaths.
        lines_before_dbload.append(
            f"cd {CONTAINER_IOC_PATH}/{bin_IOC}/iocBoot/ioc{bin_IOC}\n"
        )
        ...
        lines_before_dbload.append(
            f"{bin_IOC}_registerRecordDeviceDriver pdbbase\n\n".replace("-", "_")
        )
        ...
        if self.check_config("module", "status-ioc"):
            # st.cmd
            # lines_at_dbload
            lines_at_dbload.append(
                f'dbLoadRecords("db/status_ioc.db","IOC={self.name}")\n'
            )
            # devIocStats .db
        ...
        if self.check_config("module", "status-os"):
            ...
            # lines_at_dbload
            lines_at_dbload.append(
                f'dbLoadRecords("db/status_OS.db","HOST={self.name}:docker")\n'
            )
            # devIocStats .db
        ...
        if self.conf.has_section("RAW"):
            ...
            lines_after_iocinit.append(
                f'{self.get_config("cmd_after_iocinit", sc)}\n\n'
            )
            # file copy
    ...
    def export_for_mount(self, force_overwrite=False):
        ...
        if not self.state_manager.check_config("state", "normal"):
            print(
                f'IOC("{self.name}").export_for_mount: Failed. '
                f'Exporting operation must under "normal" state.'
            )
            return
        if not (
            self.state_manager.check_config("status", "generated")
            or self.state_manager.check_config("status", "exported")
        ):
            print(
                f'IOC("{self.name}").export_for_mount: Failed. '
        ...
        for item_file in file_to_copy:
            file_copy(
                os.path.join(self.dir_path, item_file),
                os.path.join(top_path, item_file),
                verbose=self.verbose,
            )
            os.chmod(
                os.path.join(top_path, item_file), mode=0o444
            )  # set readonly permission.
        for item_dir in dir_to_copy:
            if not dir_copy(
                os.path.join(self.project_path, item_dir),
                os.path.join(top_path, item_dir),
                self.verbose,
            ):
                print(
                    f'IOC("{self.name}").export_for_mount: Failed. '
            ...
            print(
                f'IOC("{self.name}").export_for_mount: Success. Project files {exec_type} in "{top_path}".'
            )
    ...
    def add_snapshot_files(self):
        ...
        if os.path.isfile(self.config_file_path):
            if not file_copy(
                self.config_file_path, self.config_snapshot_file, "r", self.verbose
            ):
                print(
                    f'IOC("{self.name}").add_snapshot_files: Failed, snapshot file for config created failed.'
                )
            ...
            print(
                f'IOC("{self.name}").add_snapshot_files: Failed, source file "{self.config_file_path}" not exist.'
            )
            self.state_manager.set_config("snapshot", "error")
            state_info = f"snapshot files not create correctly."
            self.state_manager.set_state_info(
        ...
        for item in os.listdir(self.src_path):
            if not file_copy(
                os.path.join(self.src_path, item),
                os.path.join(self.src_snapshot_path, item),
                "r",
                self.verbose,
            ):
                print(
                    f'IOC("{self.name}").add_snapshot_files: Failed, snapshot file '
    ...
    def restore_from_snapshot_files(self, restore_files: list, force_restore=False):
        if not isinstance(restore_files, list) or not list(filter(None, restore_files)):
            print(
                f'IOC("{self.name}").restore_from_snapshot_file: '
        ...
        if "all" in restore_files:
            ...
            if "ioc.ini" in restore_files:
                ...
                files_to_restore["config"] = files_provided[
                    "config"
                ]  # as it is to restore, get path of ioc.ini
                if not files_provided[
                    "config"
                ]:  # if it is not provided, add to unsupported list
        ...
        if not supported_file_string:
            ...
            if not force_restore:
                while not force_restore:
                    print(
                        f'IOC("{self.name}").restore_from_snapshot_file: Start.\n\n'
                    ...
                    if ans.lower() == "n" or ans.lower() == "no":
                        print(
                            f'IOC("{self.name}").restore_from_snapshot_file": Restoring canceled.'
                        )
                        return
                    if ans.lower() == "y" or ans.lower() == "yes":
                        print(
                            f'IOC("{self.name}").restore_from_snapshot_file": Execute restoring.'
                        )
                        force_restore = True
                        break
            ...
            if force_restore:
                ...
                for item in files_to_restore["src"]:
                    if not file_copy(
                        item, self.src_path, mode="rw", verbose=self.verbose
                    ):
    ...
    def generate_check(self):
        ...
        sc = "SETTING"
        for env_def in multi_line_parse(
            self.get_config(option="epics_env", section=sc)
        ):
            env_name, env_val = condition_parse(env_def)
            if not env_name:
    ...
    def check_snapshot_consistency(self, print_info=False):
        if not os.path.isdir(self.snapshot_path):
            if print_info:
                print(
                    f'IOC("{self.name}").check_snapshot_consistency": No snapshot available.'
                )
        ...
        return (
            (True, "consistent")
            if (res_config_file == 0 and res_src_dir == 0)
    ...
    def check_running_consistency(self, print_info=False):
        ...
        return (
            (True, "consistent")
            if (res_config_file == 0 and res_startup_dir == 0)
            else (False, "inconsistent")
        )
...
def gen_swarm_files(iocs, verbose):
    ...
    if verbose:
        print(
            f'gen_swarm_files: Start. Processing with iocs="{iocs}", verbosity="{verbose}".'
        )
    ...
    for service_dir in os.listdir(top_path):
        ...
        if not os.path.exists(ioc_ini_path):
            if verbose:
                print(
                    f'gen_swarm_files: Skip directory "{service_dir}" as there is no valid IOC config file.'
                )
            continue
        try:
            temp_ioc = IOC(
                dir_path=service_path,
                read_mode=True,
                verbose=verbose,
                state_info_ini_dir=os.path.join(REPOSITORY_PATH, service_dir),
                no_exec_get_src=True,
            )
            if not temp_ioc.check_config(section="IOC", option="host", value="swarm"):
                print(
                    f'gen_swarm_files: Warning. IOC "{service_dir}" not defined in swarm mode, skipped.'
                )
                continue
            if not temp_ioc.get_config(section="IOC", option="image"):
                print(
                    f'gen_swarm_files: Warning. Option "image" not defined for IOC "{service_dir}", skipped.'
                )
                continue
            else:
                ioc_settings["image"] = temp_ioc.get_config(
                    section="IOC", option="image"
                )
            # get resources settings
            ioc_settings["cpu-reserve"] = temp_ioc.get_config(
                section="DEPLOY", option="cpu-reserve"
            )
            ioc_settings["memory-reserve"] = temp_ioc.get_config(
                section="DEPLOY", option="memory-reserve"
            )
            ioc_settings["cpu-limit"] = temp_ioc.get_config(
                section="DEPLOY", option="cpu-limit"
            )
            ioc_settings["memory-limit"] = temp_ioc.get_config(
                section="DEPLOY", option="memory-limit"
            )
            # make resources dict
            reservations_dict = {}
            limits_dict = {}
            ...
            # labels
            labels_to_add = {}
            for label_line in multi_line_parse(
                temp_ioc.get_config(section="DEPLOY", option="labels")
            ):
                ...
                if k:
                    ...
                    print(
                        f"gen_swarm_files: Warning. "
                        f'Invalid label definition "{label_line}" for IOC "{service_dir}", skipped.'
                    )
        except IMIOCError as e:
            print(
                f'gen_swarm_files: Warning. Exception raised "{e}" while Parsing "{ioc_ini_path}", skipped.'
            )
            continue
        ...
        if iocs == ["alliocs"]:
            print(
                f"gen_swarm_files: Finished creating swarm files for all IOC projects."
            )
        else:
            for item in iocs:
                if item not in processed_dir:
                    print(
                        f'gen_swarm_files: Failed to create swarm files for IOC project "{item}", '
    ...
    if ioc_list:
        ...
        try:
            for ioc_item in ioc_list:
                ...
                file_copy(
                    ioc_item.config_file_path,
                    os.path.join(ioc_tar_dir, IOC_CONFIG_FILE),
                    mode="rw",
                    verbose=verbose,
                )
                file_copy(
                    ioc_item.state_manager.info_file_path,
                    os.path.join(ioc_tar_dir, IOC_STATE_INFO_FILE),
                    mode="rw",
                    verbose=verbose,
                )
                dir_copy(
                    ioc_item.src_path, os.path.join(ioc_tar_dir, "src"), verbose=verbose
                )
                # copy "project" dir if backup_mode == "all".
                if backup_mode == "all":
                    # copy from repository
                    dir_copy(
                        ioc_item.startup_path,
                        os.path.join(ioc_tar_dir, "project", "startup"),
                        verbose=verbose,
                    )
                    # copy from running data
                    ioc_run_path = os.path.join(
                        MOUNT_PATH, ioc_item.get_config("host"), ioc_item.name
                    )
                    dir_copy(
                        os.path.join(ioc_run_path, "logs"),
                        os.path.join(ioc_tar_dir, "project", "logs"),
                        verbose=verbose,
                    )
                    dir_copy(
                        os.path.join(ioc_run_path, "settings"),
                        os.path.join(ioc_tar_dir, "project", "settings"),
                        verbose=verbose,
                    )
            else:
                with tarfile.open(
                    os.path.join(backup_path, f"{now_time}.ioc.tar.gz"), "w:gz"
                ) as tar:
                    tar.add(tar_dir, arcname=os.path.basename(tar_dir))
                dir_remove(tar_dir, verbose=verbose)
                print(
                    f'repository_backup: Finished. Backup file created at {backup_path} in "{backup_mode}" mode.'
                )
        except Exception as e:
...
def restore_backup(backup_path, force_overwrite, verbose):
    ...
    try:
        ...
        print(
            f'restore_backup: Start restoring from backup file "{os.path.basename(temp_in_dir)}".'
        )
    ...
    try:
        ...
        for ioc_item in os.listdir(temp_in_dir):
            ...
            restore_flag = False
            if os.path.isdir(backup_ioc_dir) and os.path.exists(
                os.path.join(backup_ioc_dir, IOC_CONFIG_FILE)
            ):
                if ioc_item not in ioc_existed:
                    ...
                    while True:
                        ans = input(
                            f'restore_backup: "{ioc_item}" already exists, overwrite '
                        ...
                        if ans.lower() == "yes" or ans.lower() == "y":
                            ...
                            print(
                                f"restore_backup: wrong input, please enter your answer again."
                            )
                    if overwrite_flag:
                        dir_copy(backup_ioc_dir, current_ioc_dir, verbose=verbose)
                        restore_flag = True
                    ...
                    print(
                        f'restore_backup: Restoring IOC project "{ioc_item}", local project will be overwrite.'
                    )
                    dir_copy(backup_ioc_dir, current_ioc_dir, verbose=verbose)
                    restore_flag = True
            else:
            ...
            if restore_flag:
                ...
                temp_ioc = IOC(
                    dir_path=current_ioc_dir, read_mode=True, verbose=verbose
                )
