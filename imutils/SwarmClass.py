...
class SwarmManager:
    def __init__(self, verbose=False, **kwargs):
        ...
        self.services = {
            item: SwarmService(name=item, service_type="ioc")
            for item in os.listdir(REPOSITORY_PATH)
            if os.path.isdir(os.path.join(REPOSITORY_PATH, item))
        }
        for ss in GlobalServicesList:
            name = ss[0]
            if name in self.services.keys():
                print(
                    f'SwarmManager: Warning! Service "{name}" defined in GlobalServicesList '
        ...
        for ss in LocalServicesList:
            ...
            if name in self.services.keys():
                print(
                    f'SwarmManager: Warning! Service "{name}" defined in LocalServicesList '
                    f"has the same name with existing services, skipped."
                )
                continue
        ...
        for ss in CustomServicesList:
            ...
            if name in self.services.keys():
                print(
                    f'SwarmManager: Warning! Service "{name}" defined in CustomServicesList '
                    f"has the same name with existing services, skipped."
                )
                continue
    ...
    def show_digest(self, service_type=['all']):
        ...
        for item in self.services.values():
            ...
            t_l = [
                ...
                item.service_type,
                (
                    socket_info_service.get("replicas")
        ...
        for item in self.services.values():
            if item.service_type == "global":
                if item.is_available:
                    if item.is_deployed:
                        print(
                            f'SwarmManager: Skipped deploying "{item.service_name}", as it\'s been deployed.'
                        )
                        continue
                    else:
                    ...
                    print(
                        f'SwarmManager: Failed to deploy "{item.service_name}", as it\'s not available.'
                    )
    ...
    def update_deployed_services(self):
        print(
            f"Update all deployed services, this will cause all ioc services to be restarted."
        )
            ...
            if any(
                ...
                receiver_list = [
                    (
                        {
                            "name": "default-mail",
                            "email_configs": [
                                {"to": ALERT_MANAGER_RECEIVE_EMAIL_LIST[0]},
                            ],
                        }
                    ...
                    (
                        {
                            ...
                            "email_configs": [
                                {"to": email_item}
                                for email_item in ALERT_MANAGER_RECEIVE_EMAIL_LIST
                            ],
                        }
                        if ALERT_MANAGER_RECEIVE_EMAIL_LIST
                        else None
                    ...
                    (
                        {
                            ...
                            "email_configs": [
                                {"to": email_item, "send_resolved": True}
                                for email_item in ALERT_MANAGER_RECEIVE_EMAIL_LIST
                            ],
                        }
                        if ALERT_MANAGER_RECEIVE_EMAIL_LIST
                    ...
                    (
                        ...
                        if ALERT_MANAGER_DEFAULT_WEBHOOK_RECEIVER
                        else None
                    ),
                    (
                        {
                        ...
                        if ALERT_MANAGER_INFO_WEBHOOK_RECEIVER
                        else None
                    ),
                ]
    ...
    def get_deployed_swarm_services():
        result = subprocess.run(
            [
                ...
                "ls",
                "-f",
                f"label=com.docker.stack.namespace={PREFIX_STACK_NAME}",
                "--format",
                "{{.Name}}",
            ],
    ...
    def show_deployed_services():
        os.system(
            'docker stack ps -f "desired-state=running" -f "desired-state=ready" -f "desired-state=accepted" '
    ...
    def show_deployed_services_detail():
        os.system(
            "docker stack ps "
            '--format "table {{.Name}}\t{{.Node}}\t{{.DesiredState}}\t{{.CurrentState}}\t{{.Error}}\t{{.Ports}}" '
            "--no-trunc " + f" {PREFIX_STACK_NAME} "
        )

    @staticmethod
    def show_deployed_machines(show_detail=False):
        os.system(
            'docker node ls --format "table {{if .Self}}*{{end}}{{.Hostname}}{{if .Self}}*{{end}}\t'
    ...
    def backup_swarm():
        ...
        if repository_path:
            if not os.path.isdir(repository_path):
                print(
                    f"backup_swarm: $MANAGER_PATH is not a valid directory, "
                    f"backup file created at current work path."
                )
                return
        else:
            print(
                f"backup_swarm: $MANAGER_PATH is not defined, backup file created at current work path."
            )
            return
        backup_path = os.path.normpath(
            os.path.join(repository_path, "..", SWARM_BACKUP_DIR)
        )
    ...
    def restore_swarm(backup_file):
        ...
        if not os.path.isdir(f"/tmp/var/lib/docker/swarm"):
            print(
                f"restore_swarm: Failed. Backup file used for restoring is not a valid swarm backup."
            )
            return
...
class SwarmService:
    ...
    def deploy(self):
        ...
        if self.is_available:
            if self.is_deployed:
                print(
                    f'SwarmService("{self.name}").deploy_service: Service has already been deployed.'
                )
            else:
                print(
                    f'SwarmService("{self.name}").deploy_service: Service deploying ... '
                )
                command = (
                    f"cd {self.dir_path}; "
            ...
            print(
                f'SwarmService("{self.name}").deploy_service: Failed to deploy, service is not available.'
            )
            ...
            if remove_file:
                if os.path.isfile(os.path.join(self.dir_path, self.service_file)):
                    try:
                        ...
                        print(
                            f'SwarmService("{self.name}").remove_service: Remove swarm file failed.'
                        )
                    else:
                        print(
                            f'SwarmService("{self.name}").remove_service: Swarm file removed.'
                        )
        else:
            print(
                f'SwarmService("{self.name}").remove_service: Failed to remove, service is not deployed.'
            )
    ...
    def show_logs(self):
        ...
        if self.is_deployed:
            if self.service_type == "ioc" or "iocLogServer" in self.service_name:
                os.system(
                    f"docker service logs --follow --tail=1000 --raw {self.service_name}"
                )
            else:
                os.system(
                    f"docker service logs --follow --tail=1000 {self.service_name}"
                )
        else:
