...
导出后运行的IOC项目可选择进行备份操作, 将所有IOC项目在当前本地仓库和运行目录下的部分文件打包为tgz文件.

备份时可选择两种模式  
```"src"```: 仅备份IOC项目源文件及配置文件, 当恢复时IOC项目需要重新执行运行文件生成操作;    
```"all"```: 备份包括运行目录内插件模块运行时产生的文件在内的所有IOC文件, 恢复时IOC项目不需要重新执行运行文件生成操作.

1. 备份时默认将先进行IOC项目的运行检查, 确保IOC项目是最新的且是正常可运行的, 通过后方能继续备份.
   也可手动执行命令先进行运行检查   
   ```./IocManager.py exec --run-check```


2. 执行备份操作, 指定将备份文件存储位置及备份模式, 工具将自动在目标位置生成带有时间戳的备份文件   
   ```./IocManager.py exec -b --backup-path xxx [--backup-mode "src"/"all"]```


3. 执行命令进行备份文件的恢复, 指定需要恢复的备份文件及恢复模式(是否覆盖写入),
