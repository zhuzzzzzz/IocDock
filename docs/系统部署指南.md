...
0. 在配置文件 `settings.py` 中定义集群栈名称 `PREFIX_STACK_NAME`.
...
2. 生成 ansible 主机清单文件
   ...
   # 执行一般权限的 ansible 操作
   ansible all -m ping -i imtools/ansible/inventory/
...
3. 为节点打上 swarm 标签, 工具将根据标签自动执行挂载 NFS 目录的操作
