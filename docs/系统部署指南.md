...
1. 修改配置文件 `settings.py` , 在 `###### Ansible 配置 ######` 部分中定义各集群节点的主机名称, ip, 远程用户, 工作用户, 密码(密码可暂不设置).
...
3. 执行工具命令以调用 playbook 自动初始化集群环境
    ...
    ```shell
     # 调用后, 工具将创建远程用户, 设置 ssh 连接, 设置基础运行环境, 安装 docker 依赖并构建 swarm 集群
...
集群使用 NFS 来存储和共享系统公共数据和部分应用程序公共数据.
