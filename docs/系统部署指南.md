...
1. 执行命令生成服务部署文件, 更新目录配置, 然后执行部署命令
...
1. 执行命令生成服务部署文件, 确定哪些服务运行在哪些节点, 并打上节点 swarm 标签, 更新目录配置, 然后部署
...
2. 部署
    ...
    # prometheus 服务地址: http://target_ip:9090
    $ IocManager service loki --deploy
    # loki 服务地址: http://target_ip:3100
    $ IocManager service grafana --deploy
    # grafana 服务地址: http://target_ip:3000
    $ IocManager service dbwr --deploy
    # dbwr 服务地址: http://target_ip:8088/dbwr
    # pvws 服务地址: http://target_ip:8088/pvws
...
3. 打上节点标签, 更新目录设置, 执行部署
    ...
    # alertManager 服务地址: http://target_ip:9093
    ```
